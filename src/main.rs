    ...
    loop {
        ...
        for path in paths {
            ...
            if !Path::new(&dest).exists() || &filename == "__info" {
                println!("[{}] {}: {}", "IO".purple(), "File/Directory".green(), path::absolute(&refpath.clone().unwrap().path()).unwrap().display().to_string().red()); // These really should be a variable but fuck it
                println!("[{}] {}: {}", "IO".purple(), "Backup Destination".green(), path::absolute(&dest).unwrap().display().to_string().red());
